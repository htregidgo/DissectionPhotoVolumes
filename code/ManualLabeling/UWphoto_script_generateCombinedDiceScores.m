...
left_values=[2 ,3 ,4 ,10,11,12,13,17,18];
rght_values=[41,42,43,49,50,51,52,53,54];
...
for il=1:length(dlist_manLabelsAll)
    ...
    slice_dice_manual_merge = squeeze(vol_merged_manual.vol(:,:,slice_number));
    slice_dice_samseg_merge = squeeze(vol_merged_samseg.vol(:,:,slice_number));
    
    for jl=1:length(left_values)
       
        slice_dice_manual_merge(slice_dice_manual_merge==rght_values(jl))=left_values(jl);
        slice_dice_samseg_merge(slice_dice_samseg_merge==rght_values(jl))=left_values(jl);
    ...
    dice_scores_merge = dice(slice_dice_manual_merge,slice_dice_samseg_merge);
    ...
    save(pth_diceScores_mat,'dice_scores_merge','slice_dice_*','-append')
