...
%
function[segstatus,segresult]= UWphoto_function_SegmentPhotosWithSAMSEG_3Dbias(...
...
addParameter(p,'PrevRunDir','',@(x)  ischar(x))
...
prevRunDir = p.Results.PrevRunDir;
...
if ~isempty(prevRunDir) && ~exist(fullfile(prevRunDir,'temp',...
        'template.mgz'),'file')
    % run rigid alignment
    ...
    copyfile(fullfile(prevRunDir,'temp','template.mgz'),...
        fullfile(tempdir,'template.mgz'));
end
...
cmd = ['sh ',codedir,'/Segmentation/run_photoseg ',...
    ...
    '--threads ',num2str(nthread),' --template ',movedhder];
